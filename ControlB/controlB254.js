...
async function writeIfChanged() {
  const data = JSON.parse(fs.readFileSync('./Json/BlockB/DataSignalB254.json', 'utf8'));

  for (let item of data) {
    const coilAddress = 2000 + item.index254;
    const value = item.value === "1";

    // Kiểm tra nếu giá trị khác lần trước thì mới ghi
    if (prevState[coilAddress] !== value) {
      try {
        await client.writeSingleCoil(coilAddress, value);
